...
type TableParam struct {
  ...
  IdxValue []byte `thrift:"idxValue,5,required" db:"idxValue" json:"idxValue"`
  IdxValues [][]byte `thrift:"idxValues,6,required" db:"idxValues" json:"idxValues"`
...
func (p *TableParam) Read(ctx context.Context, iprot thrift.TProtocol) error {
  ...
  var issetIdxValue bool = false;
  var issetIdxValues bool = false;
  ...
  if !issetIdxValue{
    return thrift.NewTProtocolExceptionWithType(thrift.INVALID_DATA, fmt.Errorf("Required field IdxValue is not set"));
  }
  if !issetIdxValues{
    return thrift.NewTProtocolExceptionWithType(thrift.INVALID_DATA, fmt.Errorf("Required field IdxValues is not set"));
  }
  if !issetTopicKey{
...
var _elem10 int64
    ...
    p.Uuids = append(p.Uuids, _elem10)
  }
  if err := iprot.ReadListEnd(ctx); err != nil {
    return thrift.PrependError("error reading list end: ", err)
  }
